...
from lib.backend.utils import FunctionSpec, OutputType, PromptType, compile_prompt_to_md
...
def get_ai_client(model: str, **model_kwargs):
    ...
    """
    if "claude-" in model or model.startswith("c_anth/"):
...
def query(
    ...
    **model_kwargs,
    ...
    query_func = backend_anthropic.query if "claude-" in model or model.startswith("c_anth/") else backend_openai.query
