...
def backoff_create(
    ...
    try:
        ...
        logger.warning(f"Backoff retry: {e}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error in backoff_create: {e}")
        raise


def opt_messages_to_list(
    system_message: str | dict | list | None, user_message: str | dict | list | None
) -> list[dict[str, str]]:
    messages = []
    if system_message:
        # Convert dict/list prompts to string for API compatibility
