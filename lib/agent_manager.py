...
import logging
from lib.parallel_agent import ParallelAgent
from lib.journal import Journal, Node
import copy
import re
from lib.backend import query, FunctionSpec
import json
from rich import print
from lib.treesearch_utils.serialize import parse_markdown_to_dict
from lib.treesearch_utils.metric import WorstMetricValue
...
"""
        ...
        if "Related Work" in self.task_desc:
            task_desc += "Related Work:\n" + self.task_desc["Related Work"] + "\n"
    ...
    def _create_initial_stage(self):
        ...
        start_stage = int(os.environ.get("AI_SCIENTIST_START_STAGE", "1"))
        if start_stage < 1 or start_stage > 4:
            start_stage = 1

        self.current_stage_number = start_stage
        stage_name = self.main_stage_dict[start_stage]
        initial_stage = Stage(
            name=f"{start_stage}_{stage_name}_1_{'preliminary' if start_stage == 1 else 'first_attempt'}",
            description="preliminary" if start_stage == 1 else "first_attempt",
            goals=self.main_stage_goals[start_stage],
            max_iterations=self._get_max_iterations(start_stage),
            num_drafts=self.cfg.agent.search.num_drafts if start_stage == 1 else 0,
            stage_number=start_stage,
        )
        ...
        # When skipping stages, seed journal with best code from idea
        if start_stage > 1 and "Code" in self.task_desc and self.task_desc["Code"]:
            seed_code = self.task_desc["Code"]
            if len(seed_code) > 100:  # meaningful code
                from lib.treesearch_utils.metric import MetricValue
                seed_node = Node(
                    plan=f"Best code from previous experiment (stages 1-{start_stage-1})",
                    code=seed_code,
                )
                seed_node.is_buggy = False
                seed_node.metric = MetricValue(value=0.5, maximize=True, name="baseline")
                self.journals[initial_stage.name].append(seed_node)
                logging.info(f"[AgentManager] Seeded stage {start_stage} journal with {len(seed_code)} chars of code from idea")

    def _curate_task_desc(self, stage: Stage) -> str:
        task_desc = self._get_task_desc_str()

        if stage.name.startswith("3_"):
            if isinstance(self.task_desc["Experiments"], list) and len(self.task_desc["Experiments"]) > 0:
    ...
    def _create_agent_for_stage(self, stage: Stage) -> ParallelAgent:
        ...
        if main_stage == 2:
            ...
            if stage1_substages:
                ...
                best_stage1_node = None  # Continue from experiment: no stage 1 history
    ...
    def _create_stage_analysis_prompt(
        ...
        if previous_results:
            ...
            stage_number = previous_stages[-1].stage_number + 1
            notes_dir = (
                base_dir
                / "logs"
                / run_name
