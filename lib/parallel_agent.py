...
import logging
import humanize
from lib.backend import FunctionSpec, compile_prompt_to_md, query
from lib.interpreter import ExecutionResult
from lib.journal import Journal, Node
from lib.treesearch_utils import data_preview
from lib.treesearch_utils.config import Config
from lib.treesearch_utils.metric import MetricValue, WorstMetricValue
from lib.treesearch_utils.response import extract_code, extract_text_up_to_code, wrap_code
import copy
import pickle
...
class MinimalAgent:
    ...
    def parse_exec_result(
        ...
        if not isinstance(response, dict):
            logger.warning(f"parse_exec_result: API returned non-dict response: {type(response)}")
            node.analysis = str(response) if response else "API call failed - no response"
            node.is_buggy = True
        else:
            node.analysis = response.get("summary", "")
            node.is_buggy = response.get("is_bug", True) or node.exc_type is not None
        print(
    ...
    def _analyze_plots_with_vlm(self, node: Node) -> None:
        ...
        if not isinstance(response, dict):
            logger.warning(f"_analyze_plots_with_vlm: API returned non-dict: {type(response)}")
            node.is_buggy_plots = True
            node.plot_analyses = []
            node.vlm_feedback_summary = "VLM API call failed"
            return
...
class ParallelAgent:
    ...
    def _define_global_metrics(self) -> str:
        ...
        if not response or response == "":
            logger.warning("Failed to define metrics, using default")
            return "accuracy"
                ...
                try:
                    ...
                    if metrics_exec_result.exc_type is None:
                        ...
                        if not isinstance(metrics_response, dict):
                            logger.warning(f"Metrics API returned non-dict: {type(metrics_response)}")
                            child_node.metric = WorstMetricValue()
                            child_node.is_buggy = True
                        elif metrics_response.get("valid_metrics_received"):
            ...
            if not child_node.is_buggy:
                ...
                if child_node.plots:
                    try:
                        ...
                        child_node.is_buggy_plots = False  # VLM failure is non-critical
    ...
    def step(self, exec_callback: ExecCallbackType):
        ...
        for node_data in node_data_list:
            ...
            if node_data is not None and (
                ...
                new_ablation_idea = None
            elif node_data is not None and (
                self.stage_name
                and self.stage_name.startswith("4_")
                and node_data["is_buggy"] is False
            ):
