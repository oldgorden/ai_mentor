...
def compile_latex(cwd, pdf_file, timeout=30):
    ...
    tex_file = osp.join(cwd, "template.tex")
    if osp.exists(tex_file):
        with open(tex_file, "r") as f:
            tex_src = f.read()
        fixed = tex_src.replace(r"\bibliography{iclr2025}", r"\bibliography{references}")
        if fixed != tex_src:
            with open(tex_file, "w") as f:
                f.write(fixed)
            print("Fixed \\bibliography to point to references.bib")
    ...
    src_pdf = osp.join(cwd, "template.pdf")
    if not osp.exists(src_pdf):
        print("compile_latex: template.pdf not produced — LaTeX had errors.")
        return False

    try:
        shutil.move(src_pdf, pdf_file)
...
def _find_summary_in_logs(base_folder, filename):
    logs_dir = osp.join(base_folder, "logs")
    if not osp.isdir(logs_dir):
        return None
    for entry in sorted(os.listdir(logs_dir)):
        run_dir = osp.join(logs_dir, entry)
        if osp.isdir(run_dir) and entry.endswith("-run"):
            path = osp.join(run_dir, filename)
            if osp.exists(path):
                return path
    return None
...
def load_exp_summaries(base_folder):
    ...
    Searches all run directories (not just 0-run).
    """
    summary_specs = [
    ...
    loaded_summaries = {}
    for filename, key in summary_specs:
        path = _find_summary_in_logs(base_folder, filename)
        if path:
            try:
...
def filter_experiment_summaries(exp_summaries, step_name):
    ...
    for stage_name in exp_summaries.keys():
        if stage_name in {"BASELINE_SUMMARY", "RESEARCH_SUMMARY"}:
            ...
            filtered_summaries[stage_name] = {}
            ablation_data = exp_summaries[stage_name]
            if isinstance(ablation_data, list):
                for ablation_summary in ablation_data:
                ...
                for key in ablation_data.keys():
                    if key in {"best node"}:
                        filtered_summaries[stage_name][key] = {}
                        for node_key in ablation_data[key].keys():
                            if node_key in node_keys_to_keep:
                                filtered_summaries[stage_name][key][node_key] = (
                                    ablation_data[key][node_key]
                                )
...
def perform_writeup(
    ...
    try:
        ...
        if not osp.exists(osp.join(latex_folder, "template.tex")):
            shutil.copytree(
                "templates/icbinb_latex", latex_folder, dirs_exist_ok=True
            )
        ...
        try:
            vlm_client, vlm_model = create_vlm_client(vlm_model or small_model)
            desc_map = {}
        ...
        for i in range(n_writeup_reflections):
            ...
            if compile_ok and osp.exists(reflection_pdf):
                try:
                    review_img_cap_ref = perform_imgs_cap_ref_review(
                        vlm_client, vlm_model, reflection_pdf
                    )
                except Exception:
                    print("VLM img/cap/ref review failed, skipping.")
                    review_img_cap_ref = ""
                ...
                try:
                    ...
                    print("Duplicate figure detection failed, skipping.")
                    analysis_duplicate_figs = ""

                reflection_page_info = get_reflection_page_info(reflection_pdf, page_limit)
            else:
                print(f"LaTeX compilation failed for reflection {i+1}, skipping VLM review.")
                review_img_cap_ref = ""
                analysis_duplicate_figs = ""
                reflection_page_info = ""
...
"""
            ...
            if reflection_code_match:
                ...
                if reflected_latex_code != current_latex:
                    ...
                    prev_tex = current_latex
                    with open(writeup_file, "w") as fo:
                        fo.write(final_text)

                    if not compile_latex(latex_folder, reflection_pdf):
                        print("Reflection introduced LaTeX errors, rolling back.")
                        with open(writeup_file, "w") as fo:
                            fo.write(prev_tex)
                else:
            ...
            if compile_ok and osp.exists(reflection_pdf):
                ...
                try:
                    ...
                    print("VLM img selection review failed, skipping.")
...
If you believe you are done with reflection, simply say: "I am done"."""
            ...
            if reflection_code_match:
                ...
                if reflected_latex_code != current_latex:
                    ...
                    if not compile_latex(latex_folder, reflection_pdf):
                        print("Image reflection introduced LaTeX errors, rolling back.")
                        with open(writeup_file, "w") as fo:
                            fo.write(prev_tex)
                else:
                    print(f"No changes in reflection step {i+1}.")
...
USE MINIMAL EDITS TO OPTIMIZE THE PAGE LIMIT USAGE."""
        ...
        if reflection_code_match:
            ...
            if reflected_latex_code != current_latex:
                ...
                if not compile_latex(latex_folder, reflection_pdf):
                    print("Final page reflection introduced LaTeX errors, rolling back.")
                    with open(writeup_file, "w") as fo:
                        fo.write(prev_tex)
            else:
